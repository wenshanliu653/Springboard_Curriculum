...
SELECT DISTINCT 
	CONCAT(bm.surname, ', ', bm.firstname, ' booked ', f.name) AS TennisBooking
FROM (SELECT m.firstname,
    ...
    ON b.memid = m.memid
    WHERE firstname != 'GUEST') as bm
LEFT JOIN Facilities AS f
ON bm.facid = f.facid
WHERE f.name LIKE 'Tennis Court %'
ORDER BY TennisBooking;
...
SELECT 
	...
	END AS Name,
	TIME(bm.starttime) AS Time,
	f.name AS Facility,
	bm.slots AS SlotsBooked,
	CASE
		WHEN bm.memid = 0 THEN f.guestcost * bm.slots
		ELSE f.membercost * bm.slots
	END AS Cost
FROM 
	(SELECT starttime, surname, firstname, facid, m.memid, slots
     FROM Bookings AS b
     LEFT JOIN Members AS m
     ON b.memid = m.memid
...
/* Q11: Produce a report of members and who recommended them in alphabetic surname,firstname order */
        ...
        SELECT m1.surname, m1.firstname, m2.surname AS recommended_by_surname, m2.firstname AS recommended_by_firstname
        FROM Members m1
...
WHERE b.memid > 0  -- Filter out guest bookings (memid > 0 means member)
GROUP BY f.name
...
/* Q13: Find the facilities usage by month, but not guests */
        ...
        SELECT
            f.name AS Facility,
            COUNT(*),
            strftime("%m-%Y", b.starttime) AS Month
